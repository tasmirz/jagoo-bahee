...
| C7 | §1 | Mastodon: removing 5 ASes cuts LCC 92%→46% [raman2019mastodon] | **FAILS** (numbers misstated; correct figures in §10.1) |
...
## 10. Round-2 verification addendum (2026-08-22, full-text checks)

### 10.1 C7 is a factual error — the Mastodon numbers are misstated

`main.tex` §1: *"removing instances hosted in five autonomous systems from Mastodon's measured
graph cuts its largest connected component from 92% of users to 46%"* [raman2019mastodon].

What Raman et al. (IMC 2019, arXiv:1909.05801) actually report, verified in full text:

- Initially **92% of *instances*** are in the federation-graph LCC, and that LCC covers
  **96% of users** — not "92% of users".
- Removing the top-5 ASes **ranked by instances hosted** (OVH, Scaleway, Sakura, Hetzner, GMO):
  a 49% reduction leaving an LCC covering **45% of instances and 66% of users**.
- Removing the top-5 ASes **ranked by users**: the graph shatters into **272 components**.

So the sentence mixes up both ends: the 92% is an *instance* figure, and "46%" matches nothing —
the real post-removal pair is 45% instances / 66% users. A reviewer who knows this paper (it is
*the* canonical Mastodon measurement, and co-author Tyson is a prolific Fediverse researcher)
will catch it instantly, and it is load-bearing for your §1 brittleness argument. Fix, e.g.:

> "…removing the five autonomous systems hosting the most instances shrinks the federation
> graph's largest component from 92% of instances to 45% (covering 66% of users)
> [raman2019mastodon]."

Bonus from the same source you can use: 98% of instances go down at least once; a quarter for
≥1 day; 78% of instances produce <10% of their own toots ("feeder" centralization); and their
subscription-replication experiment is the closest measured argument to your redundancy design —
worth one sentence.

### 10.2 The ISI "islands" collision is closer than §6.1 stated — and comes with free allies

Full text of arXiv:2407.14427 confirms:

- Their definition: an *island* is "a group of public IP addresses partitioned from the Internet
  core, but able to communicate among themselves" — your L1/L2 concept, verbatim-adjacent.
- **They explicitly discuss national secession/shutdowns** as the policy application (Egypt 2011;
  exam-time shutdowns, citing a *Dhaka Tribune* 2018 example; US/China/Russia unplug proposals).
  So the overlap is not just vocabulary — it is the motivating scenario.
- But they are definition+measurement only: "Several systems mitigate partial outages… RON…
  Hubble… LIFEGUARD… These systems address partial reachability; **we define its scope**."
  Your one-sentence differentiation writes itself: *they define and detect the condition; this
  paper builds the system that must keep working inside it.*
- They hand you three more systems to cite and differentiate: **RON** (Andersen et al., SOSP 2001
  — overlay routing around failures), **Hubble** (Katz-Bassett et al. 2008), **LIFEGUARD** (2012).
  Your bridge differs: RON routes *around* failures among mutually reachable overlay members; your
  bridge operates when no member-to-member path exists at all and revalidates every byte it
  relays. Cite all three or a cs.NI reviewer will.

### 10.3 Verified accurate (no change needed)

- `nostrnip77`: real and accurately described — NIP-77 is "Negentropy Syncing", range-based set
  reconciliation, optional, and indeed not a base-protocol relay-to-relay exchange.
- Bibliography spot-checks all genuine: raman2019mastodon (IMC'19, 10.1145/3355369.3355572),
  bischof2023shutdowns (SIGCOMM'23 — *Destination Unreachable*, highly on-point),
  ooni2025bangladesh (*The Longest Silence*, OONI + Digitally Right), dahlberg2018aggregation
  (ARES'18), albrecht2023matrix (IEEE S&P'23), anaobi2023moderation (WWW'23).

### 10.4 Additional citable analyses (optional, strengthens §2/§9)

- arXiv:2011.06488 — formal analysis of the Matrix Event Graph replicated data type (grounds your
  "Matrix has different history assumptions" cell).
- arXiv:1910.06295 — "A Glimpse of the Matrix": measured centralization/scalability of Matrix
  federation (more evidence for your federation-brittleness argument).
- arXiv:2603.16735 — Ember: serverless E2E messaging over IPv6 mesh (2026; adjacent mesh claim
  space, one-line differentiate if cited).

**Addendum summary:** one claim upgraded to FAILS (C7 — fix the sentence), the islands related-work
fix grew by three systems (RON/Hubble/LIFEGUARD), two citations confirmed good, and the bib passed
spot-checks. Everything else in §1–§9 stands.
