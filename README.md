...
`local:up` builds and starts the backend with a transaction-capable MongoDB replica set,
Redis, and MinIO. The credentials in Compose are development-only. Use `pnpm local:logs` to
inspect the node and `pnpm local:down` to stop it. For a physical phone, change
`EXPO_PUBLIC_NODE_URL` to the computer's LAN address.
