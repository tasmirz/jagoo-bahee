...
export function CommunityCreateScreen({ colors, homeNode, onBack, onCreated }: {
  ...
  const publish = async () => {
    ...
    try {
      await createForumCommunity(homeNode.baseUrl, { name, title, description, rulesMarkdown: rules, isPrivate, isNsfw }, homeNode.discovery.services.auditLogs);
