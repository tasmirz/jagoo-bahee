...
export default function CommentEditor({ postId, parentId, onSuccess, onCancel }: { postId: string; parentId?: string | null; onSuccess?: (comment: any) => void; onCancel?: () => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      const res = await fetch('/api/comments', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: token ? `Bearer ${token}` : '' }, body: JSON.stringify({ ...payload, contentHash: Array.from(hashU8).map(b=>b.toString(16).padStart(2,'0')).join(''), userSignature: toB64(sigBytes) }) })
